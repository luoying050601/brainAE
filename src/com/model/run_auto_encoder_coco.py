...
class Autoencoder(nn.Module):
    def __init__(self):
        ...
        self.dense_enc1 = nn.Linear(3104, 1552)  # [batch*size]->[batch*size]
        ...
        self.dense_dec2 = nn.Linear(1552, 3104)
...
def run(train, val, fold=None):
    ...
    # is_1024_vanilla_0.013152188140832419.bin
    checkpoint_file = os.path.join(config.SAVE_DIR, 'is_' + config._type + '_0.013152188140832419.bin')
    checkpoint = torch.load(checkpoint_file)
    model.load_state_dict(checkpoint,False)

    model = nn.DataParallel(model, device_ids=[0,1,2,3,4,5,6,7])  # multi-GPU
    ...
    for epoch in range(config.EPOCHS):
        ...
        if fold is None:
            es(valid_loss, model, model_path=os.path.join(config.SAVE_DIR, f'is_' + config._type + '_'+str(valid_loss)+'.bin'))
        else:
